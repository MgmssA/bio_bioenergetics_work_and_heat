...
Wenn Wärmeenergie in einem System eingeleitet wird, dann führt es zur Erwärmung des Systems. Gleichzeitig wird es in der Umgebung des Systems kalt. Eine solche Reaktion nennen wir **endotherm**. Wenn ein System aber Energie abgibt, dann kühlt es selber ab. Die abgegeben Energie fließt in die Umgebung wo es wärmer wird. Solche Reaktionen nennen wir **exotherm** (Danke Paul!).
...
- $C_{p,m}$ ist die spezifische Wärmekapazität bei konstantem Druck. Sie hat die Einheit $[\jouleperkilogramperkelvin]$. Um die gesamte Wärmekapazität zu bestimmen muss mit der Masse $m$ multipliziert werden $C_p = m \cdot C_{p,m}$

- $C_{p,n}$ ist die molare Wärmekapazität und besitzt die Einheit $[\joulepermoleperkelvin]$. Die gesamte Wärmekapazität ist dann das Produkt aus Stoffmenge $n$ und molarer Wärmekapazität $C_p = n \cdot C_{p,n}$.
